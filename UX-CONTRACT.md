...
- 个人资产库不要求项目；文字/链接可多画布使用。来源声明只涉及当前展示用途，不获取生成授权；界面不再询问来源，省略时服务端记为本人创作，字段保留给日后引用/授权素材。账号隔离、认证、来源用途守卫仍由服务端强制。
- 保存队列是唯一草稿与恢复组件；完整请求先持久化，每个会话同一时刻只有一个请求在途。画布编辑（新增/删除/缩放/连线/内容/撤销）先进入本机有序 outbox 并立即投影到画布，后台按序发送；连续缩放/移动合并为一笔，尚未发出的编辑撤销即本机取消不发请求。被拒绝的画布编辑连同其后的同画布编辑一起撤回投影，等待人工处理。同键重试不重复效果；离开页面先关闭队列并排空本机写入，再释放标签页锁。

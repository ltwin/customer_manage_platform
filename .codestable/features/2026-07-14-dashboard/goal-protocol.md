# dashboard goal protocol

## 执行 loop

1. 读取 `dashboard-design.md`（approved）、`dashboard-checklist.yaml`、`goal-plan.md`、`goal-state.yaml`。
2. 进入 `cs-feat` implementation 阶段完成 checklist 7 个 steps（依赖序）；代码行为 step 默认 TDD micro-loop（RED → GREEN → VERIFY），不能 TDD 时写 `TDD exception` 和替代证据（口径见 goal-plan）。
...
6. QA passed 后进入 `cs-feat` acceptance：更新 checklist checks、items.yaml（status: done）、req `dashboard` draft→current（cs-req update）、确认 §4.3 与 OpenAPI 一致、评估 cs-keep / cs-docs。
...
- **例外（attention.md 硬约束，优先于接管规则）：git commit 必须经人工同意，不得自动执行**——实现完成后把建议的 commit message 写进报告，等 owner 执行或明确授权；禁 `git commit --no-verify`。
- **例外（attention.md）：开发前须确认当前 branch 或 worktree**——未确认前不得开始写业务代码；确认后写回 goal-plan「分支」一行。
...
- design-review residual（同类模型审查温差、ListItem 待物化、D9 条件路径）在 QA/acceptance 记录，不升级为默认阻塞。
...
- 需要改变 approved design、feature 范围、公开契约或 roadmap item（注意：D11 计划内 §4.3 ListItem 措辞更新不算）。

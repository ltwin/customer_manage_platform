...
摄影师提出链接本质是文本，画布不再提供独立链接节点创建入口；文字节点直接承载文字和 URL。已有链接节点保留内容与编辑能力，在画布按文字节点展示。此轮不迁移历史数据，不调整个人资产库的链接分类。

## 画布同步补充决策（2026-09-11）

本节记录现有画布的增量要求（摄影师本轮明确提出），不重写下文历史先导范围：画布变化通过 SSE 主动通知，取消前端固定周期的画布快照轮询；覆盖手动命令、其他窗口、独立后台任务产生的节点及执行状态变化。操作保持本地即时反馈与幂等提交，服务端快照负责最终确认。断线、页面恢复或数据库监听恢复后读取最新状态补齐，不要求历史事件逐条回放；账号隔离与现有登录认证必须沿用。机器契约见 OpenAPI 的 watchCreativeCanvas，实现与验证见 `docs/dev/creative-text-canvas.md` 的 SSE 同步章节。

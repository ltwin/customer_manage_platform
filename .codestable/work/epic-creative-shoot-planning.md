...
- 2026-08-21：低优先级清尾批（11 条一次收口，38 条真差距全部关闭，待提交）：摄取——顶栏「已选 N 条」chip（仅确认候选步）、保存摘要附「镜头 X · 准备项 Y」明细与「基于策划第 N 版提交」说明、409 冲突拉取服务端最新会话并展示可展开差异面板（版本漂移/候选与链接保留计数/原文校验和，staleDiffLines 纯函数落 ingestionConflict.ts，内容一致不弹）、「全选」一键恢复手动丢弃；Run——页头 run-meta 标签行（capture 模式/第 N 版/15s 低频时钟；「第几场」因契约无 session 序列字段如实不展示）、参考区空态「本镜未绑定参考素材」+「原作素材仅用于现场比对，不做生成」、跳过原因 4 处措辞对齐原型（准备物料缺失/时间不够/主体不可用/创作方向变更，Run 选择器与 outcomeLabels 同源）；工作台——时间窗面板来源状态行四态（windowSourceNote 按投影 status 派生，执行入口仍在 CRM 关联卡）、经营草稿逐行表加套系基准价行/依据列（source_fact.field · rule_key）/建议总价行 + 订单应用确认带金额前后对照、撤销认领确认加「核对提醒一并撤销」（后端撤销确实预留 assignment_revoked 提醒世代）+ 认领条目现场缺失/待核对标签（工作台同源派生传入）+ 认领规则说明、公开规模字段说明与「进度怎么流转」四步面板、台账页一键空白建案（默认 未命名策划/待补充，移除必填弹窗）；分享——document.title 随档位/失效态变化、页脚生命周期告知、昵称隐私说明、锁定占位改摄影师第一人称。纯前端批次，无契约/后端改动。验证：test:shoot-planning 55/55（新增 7 条，3 条旧断言随实现更新：跳过措辞、订单确认标题、台账建案幂等键）、test:plan-ingestion 16/16（新增 3 条）、test:plan-share 16/16（新增 1 条）、前端全量 249 中 248 绿（1 失败为 account-center A8 存量，前批已归因）、planning-prototype-v2 6/6、prototype-contract 4/4、lint 0 error、build 通过。台账终态：38 条真差距全部收口（36 完整修复 + WS-04/WS-07 部分收口）；残余挂账仅 WS-04 侧栏扩展标签（D2 不做）、WS-07 纯链接素材（owner 决策另立项）与 plan-share S6/S7 证据残差；e2e 完整跑仍挂账。
- 2026-08-21：收口后两处 UI 修复（owner 报障）：① 创作 Brief 页「进度怎么流转」提示块误置于两列网格首格，导致 BriefEditor 被挤进窄侧栏并出现大段空白——移出网格改为整行块，测试加「提示必须在网格外」回归断言；② 参考素材面板按原型重构为上传卡 + 素材墙双面板（控件横排自适应网格、素材墙 auto-fill minmax(180px) 自适应、卡内 图→名→标签→单行 hint 层级、挂载操作从三控件纵叠收敛为下拉整行 + 双按钮并排），清除 planning.css 中互相覆盖的媒体控件补丁（445/467 两段重复定义）。验证：planning-media 8/8（新增布局回归断言）、shoot-planning 55/55、全量 250 中 249 绿（1 为 account-center 存量）、mjs 10/10、lint 0 error、build 通过。

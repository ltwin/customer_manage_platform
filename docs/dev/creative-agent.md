...
B3 基线为 `0d316fd`。C 增加迁移 0049（等待标识、等待原因、终态恢复来源），
控制端口及轮询面板。沿用 FND-07 原定边界：SSE/消息分块与画布写工具在 FND-08，附件在 FND-09。

### 控制与救援

- `CancelRun` 按 run 身份幂等取消，不要求面板追赶每一次 revision。取消事务立即废止 epoch。
  已派发/unknown 请求转 reconciling，保留 slot 和未知预算；没有未知调用则关闭并释放占用。
  `CloseReconciliation` 允许明确结束等待，但不将未知费用记为零，Gateway 继续保存核算证据。
- 控制事务按 slot → Gateway group / 各预算桶 / request → run 加锁。Gateway 的
  `LockCallerGroupInTx` 只暴露调用事实，不读取 Harness 表；迟到消费以原 claim 再次判断执行权。
  终态只释放已知结果的 pending consumer，未知调用继续保留。费用更新不会重开终态。
- Worker 用数据库时钟检查租约，续租也核对 epoch，失权或续租失败后取消当前执行并等待续租协程退出。
  实际模型结果已到但账号已取消时，不再发布新的 assistant 消息。
- `creative-worker` 启动时立即、之后每 15 秒执行一页账号救援，包含停用账号的遗留运行。
  过期运行关闭；旧租约且有兼容恢复证据的运行原子重新入队；未知调用只核实。
  已交付且全部步骤完成可以直接结束；有步骤但无 checkpoint 的普通执行明确停止，不从原问题重开。
  每次扫描有批次和超时上限，超时游标停在最后实际尝试的账号，不跳过未处理后缀；Gateway 无供应商查询能力时不会伪造已核实结论。

### 等待与终态恢复的当前能力

- Eino 真实中断且已保存 checkpoint 时可进入 waiting_input，轮换 waiting_token 并释放 slot。
  当前三个只读工具仍不会主动向摄影师提问；后续业务等待工具接入同一控制出口，不能仅凭一段模型文字设置等待。
- `SupplementRun` 本阶段接收文字补充，校验 revision、waiting_token、原同意及原截止时间。
  新增独立输入批次和摄影师消息，同事务抢占 slot / 入队；不更换模型、Skill 或 consent。
  Eino middleware 按批次给恢复状态追加消息，重放不重复追加，也不改历史输入。
  引用范围扩张需新同意和明确新任务；附件/画布提案采纳不在此入口混入。
- `RetryRun` 新建独立 run 和 trigger 消息，保存 source_run_id/source_summary 及 retry_of_step_id。
  当前一次只恢复一个没有持久结果的 `read_skill_resource`，复制原参数、固定 Skill 包与来源步骤身份，
  经原只读工具路径执行，不再向模型发送原问题。恢复链指向原步骤；已成功恢复的步骤不再次执行。
  `ReadRunResult` 的定位符属于原 run，不静默换绑；模型、成功、未知、过期或缺失证据的步骤均拒绝自动恢复。
  Gateway 已关闭的模型请求不能靠新 run 换收费身份；画布写步骤未来由其正式回执适配器提供恢复证明。
- 0049 有等待/恢复证据时拒绝有损回滚；清空状态前应导出，不能用降级绕过记录。

### REST 与面板

新增最近 30 次运行列表、步骤摘要、取消、结束核实等待、文字补充和受限步骤恢复端点，OpenAPI 已生成两侧类型。
创作台的“创作助手”入口提供会话创建/选择、模型和固定 Skill 选择、明确外发同意、文字发送与持久消息阅读。
默认 2 秒轮询，后台标签 15 秒；切换会话和关闭面板撤销旧读取，HTTP 断线不取消运行。
费用待核实与运行终态分别显示。初版展示最近消息与运行，完整历史翻页和流式体验由后续面板完善。

提交前将完整命令及 operation_id 写入按账号/画布隔离的 sessionStorage。响应未知时保留原操作，
刷新后明确“确认原操作”仍发送同一身份；授权发送前固定后续 run 的 operation_id 与创建时间，授权成功后保存完整 run 命令再发送。
关闭面板后旧实例停止推进；替换/删除存储须匹配当前完整命令。确定拒绝先恢复原文、模型、Skill、会话再清 pending。
同意使用 selected_revisions 空集合：只允许本会话文字及选定 Skill，不授权任何素材修订，
不扩大为 account_library。引用未列出的修订仍被服务端拒绝。

### 验证证据

- 真 PostgreSQL / Gateway / Eino：排队取消幂等、跨账号拒绝、未知保留和结束等待、无 checkpoint 停止、
  旧租约恢复、等待补充并保持原期限、取消后迟到结果不发布、固定 Skill 只读步骤恢复不增加模型调用。
- `make check-go` 全量通过；`make check-frontend` 385 项通过；`make generate-check` 无漂移。
- Playwright 使用真实组件与模拟 HTTP：响应丢失后刷新并确认，2 次 POST 只有 1 个逻辑 run；
  切换会话不串消息、取消/结束等待保留未知费用；1440×960 与 390×844 无水平溢出、无浏览器异常。
- `CREATIVE_AGENT_LIVE=1` 显式开启的 `TestCDeepSeekLiveHarness` 已通过：真实 DeepSeek → Gateway → Eino
  → assistant 消息落库、费用 settled、槽位释放；队列重投没有第二个模型请求。隔离账号最高 0.05 USD，
  模型输出上限 128 tokens，使用固定验收文本，不读取真实项目内容。普通门禁默认跳过该付费测试。

C 首轮审查修复增加：面板原样文字授权 payload 的真实服务验收及未授权素材拒绝；恢复队列到期/模型停用仍保留
Gateway 未结费用；数据库锁阻塞至扫描超时后分页不跳账号。`scripts/creative-agent.e2e.mjs` 在 Vite 前端上
复现真实组件的两种授权响应顺序（收起重开后只创建一个运行），以及未知提交→刷新→422 后草稿完整可编辑。
脚本使用模拟 HTTP，不调用供应商；启动 Vite 后可用 `CREATIVE_AGENT_URL` 指定地址运行。

...
const navItems = [
  ...
  { key: 'planning', label: '创意空间', to: '/creative', icon: PlanningIcon },
...
export default function AppShell() {
  ...
  const studio = location.pathname === '/creative' || location.pathname.startsWith('/creative/')
  ...
  useEffect(() => {
    ...
    setTimezoneState((current) =>
      beginPageRead(current, '正在加载账号时区', true),
    )
    fetchMe()
      .then((account) => {
        if (!active) return
  ...
  const timezoneError =
    timezonePresentation.notice?.kind === 'error' ||
    timezonePresentation.notice?.kind === 'refresh-error'
  ...
  return (
    <AccountCenterProvider theme={theme} setTheme={setTheme} notify={notify}>
      <div className={studio ? 'app-shell app-shell--studio' : 'app-shell'}>
        {!studio && (
        ...
        <div className="main">
          {timezonePresentation.notice && (
            <StateNotice {...timezonePresentation.notice} />
          )}
      ...
      {!studio && <AccountMenu slot="mobile" />}

...
export default function ShootPlanIngestionPage() {
  ...
  return <>
    <header className="topbar">
      ...
      <div className="topbar-actions">{step === 'review' && <span className="tag tag-accent">已选 {keptCount} 条</span>}<Link className="btn" to={`/shoot-plans/${encodeURIComponent(id)}`}>返回工作台</Link>{session?.state === 'editing' && <button className="btn btn-danger-ghost" disabled={busy} type="button" onClick={() => void abandon()}>结束本次摄取</button>}</div>
    </header>
...
function CommitStep({ session, candidates, shotCount, readinessCount, planRevision, referenceCount, assetCount, busy, onCommit, onBack, onWorkspace }: { session: PlanIngestionSession | null; candidates: Candidate[]; shotCount: number; readinessCount: number; planRevision: number; referenceCount: number; assetCount: number; busy: boolean; onCommit: () => void; onBack: () => void; onWorkspace: () => void }) {
  ...
  return <section className="planning-panel ingestion-commit"><div className="panel-heading"><div><h2>{committed ? '已保存摄取结果' : finished ? '本次摄取已结束' : '确认保存'}</h2><p className="muted">所有核心候选、参考链接和素材绑定会一次提交；失败时不会产生半条镜头。</p></div><span className={`tag ${committed ? 'tag-ok' : finished ? 'tag' : 'tag-accent'}`}>{committed ? '已保存' : finished ? '已结束' : '待提交'}</span></div><div className="ingestion-summary-grid"><div><b>{candidates.length}</b><span>核心候选</span><small>镜头 {shotCount} · 准备项 {readinessCount}</small></div><div><b>{referenceCount}</b><span>参考链接</span></div><div><b>{assetCount}</b><span>素材绑定</span></div></div><p className="ingestion-field-hint">基于策划第 {planRevision} 版提交；保存成功后策划会生成新版本，执行历史可回溯。</p>{finished ? <button className="btn btn-primary" type="button" onClick={onWorkspace}>返回策划工作台</button> : <div className="ingestion-side-actions"><button className="btn" type="button" onClick={onBack}>继续编辑</button><button className="btn btn-primary" disabled={busy || !session} type="button" onClick={onCommit}>确认保存</button></div>}</section>
}

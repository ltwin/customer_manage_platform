...
export default function ShootPlanIngestionPage() {
  ...
  async function uploadReference(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      const result = await uploadPlanAsset(id, plan.revision, file, {
        source_class: 'customer_supplied', rights_basis: 'display_consent', license_generation_reference_granted: false,
      }, 'moodboard_display', newPlanningMutationKey('ingestion-asset'))
  ...
  function buildCommitInput(): CommitInput {
    const shots: ShotDecision[] = candidates.filter((candidate) => candidate.kind === 'shot').map((candidate) => candidate.action === 'discard'
      ? { candidate_id: candidate.candidate_id, action: 'discard', reason: '摄影师已丢弃' }
	      : { candidate_id: candidate.candidate_id, action: 'keep', client_ref: candidate.candidate_id, shot: { title: candidate.title, notes: candidate.normalized_content } })
  ...
  async function previewEdits() {
    ...
    try {
      const next = await previewPlanIngestionSession(id, session.id, {
		...
		content_overrides: candidates.map((candidate) => ({ candidate_id: candidate.candidate_id, kind: candidate.kind, title: candidate.title, action: candidate.action === 'discard' ? 'discard' : 'keep', acknowledge_source_change_revision: candidate.acknowledged_source_change_revision ?? undefined })),

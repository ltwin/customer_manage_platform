...
import { staleDiffLines, type IngestionDiffSide } from './ingestionConflict'
...
export default function ShootPlanIngestionPage() {
  ...
  const [staleServerSession, setStaleServerSession] = useState<PlanIngestionSession | null>(null)
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(cause instanceof ApiError && cause.status === 404 ? '策划或整理会话不存在，或不属于当前账号。' : planningErrorMessage(cause, '「从聊天整理」加载失败'))
  ...
  function markStale() {
    setStale(true)
    if (!session) return
    void getPlanIngestionSession(id, session.id)
      .then((fresh) => { if (fresh.revision !== session.revision || fresh.source_checksum !== session.source_checksum) setStaleServerSession(fresh) })
      .catch(() => undefined)
  }

  async function parseSource() {
	    if ((!source.trim() && assets.length === 0) || !plan) return
    setBusy(true); setError(null); setStale(false); setStaleServerSession(null)
    try {
      const next = session
      ...
      if (cause instanceof ApiError && cause.code === 'ingestion_revision_conflict') markStale()
  ...
  function selectAllKept() {
    setCandidates((current) => current.map((candidate) => candidate.action === 'discard' ? { ...candidate, action: 'keep' as const } : candidate))
    notify('已全选')
  }
  ...
  async function abandon() {
    ...
    try {
      ...
      applySession(next)
      notify('本次整理已结束，原文与素材仍按保留规则可恢复查看。')
    } catch (cause) { setError(planningErrorMessage(cause, '结束整理失败')) } finally { setBusy(false) }
  }

  if (loading) return <main className="content planning-content ingestion-page"><div className="planning-empty"><strong>正在打开「从聊天整理」</strong><span>正在恢复原文和候选。</span></div></main>
  ...
  return <>
    <header className="topbar">
      <div><div className="crumb"><Link to={`/shoot-plans/${encodeURIComponent(id)}`}>拍摄策划</Link> / 从聊天整理</div><h1>{plan.title} · 从聊天整理</h1></div>
      <div className="topbar-actions">{step === 'review' && <span className="tag tag-accent">已选 {keptCount} 条</span>}<Link className="btn" to={`/shoot-plans/${encodeURIComponent(id)}`}>返回工作台</Link>{session?.state === 'editing' && <button className="btn btn-danger-ghost" disabled={busy} type="button" onClick={() => void abandon()}>结束本次整理</button>}</div>
    </header>
    <main className="content planning-content ingestion-page">
      <div className="ingestion-steps" aria-label="整理步骤">
      ...
      {stale && staleServerSession && session && (() => {
        const localSide: IngestionDiffSide = {
          revision: session.revision,
          keptCandidates: activeCandidates.length,
          totalCandidates: candidates.length,
          keptReferences: keptReferenceCount,
          totalReferences: references.length,
          sourceChecksum: session.source_checksum,
        }
        const serverSide: IngestionDiffSide = {
          revision: staleServerSession.revision,
          keptCandidates: staleServerSession.candidate_snapshot.content_candidates.length,
          totalCandidates: staleServerSession.candidate_snapshot.content_candidates.length,
          keptReferences: staleServerSession.candidate_snapshot.reference_link_candidates.length,
          totalReferences: staleServerSession.candidate_snapshot.reference_link_candidates.length,
          sourceChecksum: staleServerSession.source_checksum,
        }
        return <details className="ingestion-stale-diff"><summary>查看本地与服务端的差异</summary><ul>{staleDiffLines(localSide, serverSide).map((line) => <li key={line}>{line}</li>)}</ul></details>
      })()}
      {confirmingReparse && (
        <ConfirmDialog
          title="重新解析并重建候选？"
...
function ReviewStep({ candidates, references, dropped, readinessCandidates, shotCandidates, readinessLinkCandidates, readinessLinkAcks, referenceAcks, links, setLinks, onAcknowledgeReadinessLink, onAcknowledgeReference, onEdit, onDiscard, onRestore, onMerge, onSelectAll, onReferenceToggle, onReferenceEdit, assets, selectedAssets, setSelectedAssets, onNext, onBack }: { candidates: Candidate[]; references: components['schemas']['IngestionReferenceLinkCandidate'][]; dropped: components['schemas']['IngestionDroppedCandidate'][]; readinessCandidates: Candidate[]; shotCandidates: Candidate[]; readinessLinkCandidates: components['schemas']['IngestionReadinessLinkCandidate'][]; readinessLinkAcks: Record<string, number>; referenceAcks: Record<string, number>; links: Record<string, string[]>; setLinks: (value: Record<string, string[]>) => void; onAcknowledgeReadinessLink: (candidateID: string, revision: number) => void; onAcknowledgeReference: (candidateID: string, revision: number) => void; onEdit: (id: string, patch: Partial<Candidate>) => void; onDiscard: (id: string) => void; onRestore: (item: components['schemas']['IngestionDroppedCandidate']) => void; onMerge: (index: number) => void; onSelectAll: () => void; onReferenceToggle: (id: string) => void; onReferenceEdit: (id: string, patch: Partial<components['schemas']['IngestionReferenceLinkCandidate']>) => void; assets: PlanAsset[]; selectedAssets: Set<string>; setSelectedAssets: (value: Set<string>) => void; onNext: () => void; onBack: () => void }) {

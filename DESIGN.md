...
owner明确要求正式工作台遵循v5原型，不受旧CRM朴素外观限制。`/creative`采用独立的全屏画布布局，CRM导航通过左上角返回入口连接；不显示双侧CRM导航。其他CRM页面的现有样式不因该路由改变。

唯一运行时主题为`frontend/src/creative-canvas/editor/workspace.css`：背景#141619，文字#eeeee9，辅助文字#a8adb5，鼠尾草灰绿强调#bdcec9；玻璃使用#22262cde底色、28px模糊、细描边与顶部内高光，阴影建立浮层深度。正文沿PingFang/system字体，品牌与空画布标题用Songti；内容节点20px以内圆角、克制的绿色选中光晕。共享btn/input在该路由通过主题变体呈现，不复制交互逻辑。

画布是主体，顶部栏68px，左侧悬浮工具轨与可收起资产库，右侧按需打开编辑面板。工具和面板不随保存提示跳动。窄屏采用覆盖式面板而非长页面堆叠；可随时收起回到画布。仅呈现已经实现的功能，不用假Agent或生成按钮填充画面。

...
func TestUnregisteredAPIPathsReturn404Envelope(t *testing.T) {
	...
	cases := []struct{ method, path string }{
		{http.MethodGet, "/api/v1/reminders"},   // 未实现域端点（schedule-calendar 范围守护）
		{http.MethodGet, "/api/v1/dashboard"},   // 未实现域端点（schedule-calendar 范围守护）
		{http.MethodGet, "/api/v1/settings"},    // 未实现域端点（schedule-calendar 范围守护）
		{http.MethodGet, "/api/v1/export"},      // 未实现域端点（schedule-calendar 范围守护）

...
func TestAttemptLimiterMigrationCreatesVersionedBudgetTable(t *testing.T) {
	...
	for index, label := range []string{
		"execution-continuations", "agent-controls", "agent-checkpoints", "agent-context", "agent-runs", "skill-foundation", "agent-conversations", "llm-gateway",
...
func TestAuthReadinessInspectsCurrentLimiterSchemaAndLegacyCutover(t *testing.T) {
	...
	if state.SchemaVersion != int(migrationTip(t)) || !state.DatabaseReady || !state.LimiterSchemaReady || !state.LegacyCutoverReady {

ALTER TABLE customers
    ADD COLUMN avatar_revision BIGINT NOT NULL DEFAULT 0 CHECK (avatar_revision >= 0),
    ADD COLUMN avatar_version TEXT,
    ADD COLUMN avatar_object_id TEXT,
    ADD COLUMN avatar_media_type TEXT,
    ADD COLUMN avatar_size BIGINT,
    ADD COLUMN avatar_updated_at TIMESTAMPTZ,
    ADD CONSTRAINT customers_avatar_pointer_complete_check CHECK (
        (
            avatar_version IS NULL
            AND avatar_object_id IS NULL
            AND avatar_media_type IS NULL
            AND avatar_size IS NULL
            AND avatar_updated_at IS NULL
        )
        OR
        (
            avatar_version IS NOT NULL
            AND avatar_object_id IS NOT NULL
            AND avatar_media_type IS NOT NULL
            AND avatar_size IS NOT NULL
            AND avatar_updated_at IS NOT NULL
            AND avatar_version ~ '^sha256-[0-9a-f]{64}$'
            AND avatar_object_id ~ '^[0-9a-f]{32}$'
            AND avatar_media_type IN ('image/jpeg', 'image/png', 'image/webp')
            AND avatar_size > 0
            AND avatar_size <= 5242880
        )
    );

CREATE TABLE avatar_object_gc (
    account_id       TEXT        NOT NULL REFERENCES accounts (id),
    customer_id      TEXT        NOT NULL,
    avatar_version   TEXT        NOT NULL CHECK (avatar_version ~ '^sha256-[0-9a-f]{64}$'),
    avatar_object_id TEXT        NOT NULL CHECK (avatar_object_id ~ '^[0-9a-f]{32}$'),
    not_before       TIMESTAMPTZ NOT NULL,
    next_attempt_at  TIMESTAMPTZ NOT NULL,
    attempts         INTEGER     NOT NULL DEFAULT 0 CHECK (attempts >= 0),
    last_error       TEXT,
    created_at       TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_at       TIMESTAMPTZ NOT NULL DEFAULT now(),
    PRIMARY KEY (account_id, customer_id, avatar_object_id),
    FOREIGN KEY (account_id, customer_id) REFERENCES customers (account_id, id)
);

CREATE INDEX avatar_object_gc_due_idx
    ON avatar_object_gc (account_id, next_attempt_at, customer_id, avatar_object_id);

CREATE TABLE avatar_reconciliation_checkpoint (
    account_id                 TEXT   PRIMARY KEY REFERENCES accounts (id),
    object_inventory_cursor    TEXT   NOT NULL DEFAULT '',
    object_inventory_cycle     BIGINT NOT NULL DEFAULT 0 CHECK (object_inventory_cycle >= 0),
    pointer_customer_id_cursor TEXT   NOT NULL DEFAULT '',
    pointer_cycle              BIGINT NOT NULL DEFAULT 0 CHECK (pointer_cycle >= 0),
    updated_at                 TIMESTAMPTZ NOT NULL DEFAULT now()
);
